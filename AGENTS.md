...
- Play only serves Wear apps to watch device profiles; apkeep's built-in list
  has none. `apkeep -a net.kusik.wristchess -d google-play -o device=twa,device_properties_file=$PWD/twa.properties,split_apk=1 -e <email> -t <aas_et token> --accept-tos <dir>`.
- Token: EmbeddedSetup login → DevTools cookie `oauth_token` (`oauth2_4/…`,
  single-use, minutes) → `apkeep -e <email> --oauth-token …` prints the
  long-lived `aas_et/` token. Stored in secretspec as **GPLAY_AAS_TOKEN_DIOVA**
  and **GPLAY_AAS_TOKEN** (account diova.eid@gmail.com; needs a Play profile —
...
- Google Drive: APKs go to My Drive `A-Z/Software/APKs`
  (folder id `1GPyrPJ0wU7-Bm6bNTYkYW5zTe3NPLJ5F`). Both APKs uploaded 2026-09-20 via
...
- 
  the arm64 shim + scripts are ours (GPLv3 via Fairy-Stockfish). APKs may be published to the public mod repository releases.
